...
- Subscriptions：订阅池新增、刷新、启停、节点地区修正、别名、标签、`reject_regex` 保存；禁用池不允许刷新，最近拉取成功时显示灰色相对时间，失败时显示红色相对时间，悬停可查看本地绝对时间。
...
- Traffic：真实请求日志展示、详情抽屉、分页加载和 CSV 导出；移动端底部加载按钮保持单行固定高度。
- System：运行统计展示、Proxy token 生成、配置重载、日志级别与请求日志保留天数保存、健康检查默认值、Proxy 重试设置、当前配置导出。
...
桌面端保持左侧固定导航和宽表格信息密度。移动端使用顶部横向导航，页面头、表单、统计和弹窗切换为单列；数据表保留列结构并在卡片内横向滚动，避免主页面横向溢出。页面通过 viewport 限制禁止缩放，从而允许输入控件继续使用紧凑字号且不触发 iOS Safari 聚焦自动放大。
