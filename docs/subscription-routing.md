...
每次成功拉取还会把解析后的原始节点写入配置目录下的 `.runtime/subscriptions.json`。缓存键是订阅 URL 的 SHA-256，不保存订阅地址本身；节点凭据仍属于敏感信息，因此目录权限为 `0700`、文件权限为 `0600`。进程或容器重启后会照常尝试实时拉取；只有实时拉取失败时才按 URL 哈希读取缓存、重新应用当前 `filter`、`reject_regex` 和节点元数据，然后恢复可用节点。本次拉取仍标记为失败，不会把缓存恢复伪装成拉取成功。
