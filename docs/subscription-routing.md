...
- `enabled`：是否启用该节点池；设为 `false` 时，启动、配置重载、手动刷新和定时任务均不拉取该订阅。
...
每次成功拉取还会把解析后的原始节点写入配置目录下的 `.runtime/subscriptions.json`。缓存键是订阅 URL 的 SHA-256，不保存订阅地址本身；节点凭据仍属于敏感信息，因此目录权限为 `0700`、文件权限为 `0600`。进程或容器重启后，已启用的订阅会照常尝试实时拉取；只有实时拉取失败时才按 URL 哈希读取缓存、重新应用当前 `filter`、`reject_regex` 和节点元数据，然后恢复可用节点。本次拉取仍标记为失败，不会把缓存恢复伪装成拉取成功。禁用订阅不发起启动、手动或定时拉取。
...
每个 subscription 池在运行时记录最近一次拉取尝试完成的时间和结果，覆盖以下入口：

- 服务启动和配置重载时的初次拉取
- Admin 手动刷新
- `update_interval` 触发的自动刷新

`GET /api/nodes` 对订阅池返回：

- `last_updated`：UTC RFC3339 时间
- `last_refresh_failed`：最近一次拉取或过滤是否失败
...
最近尝试时间和成功/失败状态保存在进程内存中，不写入 `nodes.yaml`。服务启动会立即拉取已启用的 subscription 池；若启动拉取失败但存在上一次成功缓存，新进程恢复缓存节点，同时以本次失败结果建立状态。禁用池保持空闲状态，不产生拉取失败日志。
