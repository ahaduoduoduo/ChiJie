...
│   ├── pool/
│   │   ├── manager.go           # 节点池：静态/模板/订阅/直连池管理、出口选择、模板优先级、家宽降级和离线单节点尝试
...
│   │   ├── data.jsx             # 地区名称、ISO-2 自动旗帜、地区组和模拟数据辅助函数
...
│   │   ├── page-subscriptions.jsx # Subscriptions：订阅新增、刷新、最近拉取结果、启停、本地地址许可、元数据、reject_regex 保存和窄屏布局
